...
public class DroneJobService {
    ...
    private List<Drone> transitionState(State previous, State next) {
        List<Drone> drones = droneRepository.findAllByState(previous.toString());
    ...
    private void checkDroneBattery() {
        ...
        List<Drone> drones = droneRepository.findAllByState(State.RETURNING.toString());
        if (drones.size() == 0) {
            return;
        }
        for (Drone drone : drones) {
    ...
    public void startCharging() {
        ...
        List<Drone> drones = droneRepository.findAllByState(State.IDLE.toString());
        if (drones.size() == 0) {
            return;
        }
        for (Drone drone : drones) {
